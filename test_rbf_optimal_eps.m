...
for i=1:length(grids)
    ...
    [best_eps(i), best_scores(i)] = fminbnd(@(x) rbf_eps_score(nodes_dataset, x, 0, options), 0.1, 4);
end
...
for i=1:length(grids)
    ...
    [best_eps(i), best_scores(i)] = fminbnd(@(x) rbf_eps_score(nodes_dataset, x, 1, options), bounds(i,1), bounds(i,2));
end
...
for i=1:length(grids)
    nodes_dataset = sprintf(options.base_dataset, grids(i));
    [best_eps(i), best_scores(i)] = fminbnd(@(x) rbf_eps_score(nodes_dataset, x, 2, options), bounds(i,1), bounds(i,2));
end
...
for i=1:length(grids)
    nodes_dataset = sprintf(options.base_dataset, grids(i));
    [best_eps(i), best_scores(i)] = fminbnd(@(x) rbf_eps_score(nodes_dataset, x, 3, options), bounds(i,1), bounds(i,2));
end
